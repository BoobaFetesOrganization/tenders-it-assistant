...
$tenantId = $login.tenantId; #the tenant ID in which the Data Collection Endpoint resides
$appId = $login.appId; #the app ID created and granted permissions
$appSecret = $login.appSecret; #the secret created for the above app - never store your secrets in the source code
##### >>>> END <<<<<
...
if ("file" -eq $Type) {
    ...
    if (-not $payload -is [array]) { throw "Axel Roland-Gosselin : soit vous reactiver la ligne ci-dessus et installer powershell 7 et l'utilisez pour ce script soit il faut se débrouiller pour avoir @payload sous forme de tableau" }
